...
#爬取“古诗文网”的古诗，并保存到txt文件中。
# 2.0版独立于1.0版，可以对爬取目标中含有“so”的网页进行爬取。
#2.0版可以提取网页主题作为txt文件的题目。
#3.0版综合了前两版的特点，可以自主判断是否含有“so”。
#不足之处：如果待爬取的网页含有多个box，则只能提取第一个内容。
...
print "本程序可以爬取“古诗文网”中的古诗词，并保存为txt文档。"
print "请到http://www.gushiwen.org/网站查找需要提取的诗词网页地址。"
print "请输入需要爬取的网址"
url = str(raw_input(""))  # 爬取目标

if "/so." in url:
    addurl="http://so.gushiwen.org"
else:
    addurl=""
parser = 'html.parser'
gushi_text=""  #定义参数
